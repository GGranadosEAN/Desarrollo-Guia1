...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        String[] zonas = {"Zona norte", "Zona sur", "Zona oriente", "Zona occidente", "Zona centro"};
        int maxNumExpo = 0;
        String zonaMaxNumExpo = "No hay expositores en las zonas";
        for (String zona : zonas) {
            if (maxNumExpo < buscadorZona(zona)) {
                maxNumExpo = buscadorZona(zona);
                zonaMaxNumExpo = zona;
            }

        }
        return "Zona [" + zonaMaxNumExpo + "] Numero de Expositores [" + maxNumExpo + "]";
    }

    public int buscadorZona(String zona) {
        int maxNumExpo = 0;
        for (Puesto puesto : puestos) {
            if (puesto.estaOcupado() && puesto.darZona().equals(zona)) {
                for (Empresa empresa : empresas) {
                    if (empresa.darTipo().toString().equals("Expositora") && empresa.darNombre().equals(puesto.darNombreEmpresa())) {
                        maxNumExpo = maxNumExpo + empresa.darNumeroPersonasAsistentes();
    ...
    public String metodo2() {
        ...
        double numExpo = 0;
        for (String zona : zonas) {
                numExpo = numExpo + buscadorZona(zona);
        }
        double maxExpFeria = MAX_PERSONAS_CENTRO * 8 +
                MAX_PERSONAS_NORTE * 4 +
                MAX_PERSONAS_OCCIDENTE * 3 +
                MAX_PERSONAS_ORIENTE * 3 +
                MAX_PERSONAS_SUR * 4;
        var result = numExpo/maxExpFeria;
