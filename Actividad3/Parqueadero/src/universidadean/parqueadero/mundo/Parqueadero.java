...
public class Parqueadero {
    ...
    public void avanzarHora() {
        ...
        if (HORA_INICIAL <= horaReal && horaReal <= HORA_CIERRE) {
            abierto = true;
        } else {
            abierto = false;
        }
    ...
    public String metodo1() {
        int maxTime = 0;
        String placaMaxTime = "Ningun auto a acumulado tiempo";
        for (Puesto puesto : puestos) {
            if (puesto.darCarro() != null && puesto.estaOcupado() && maxTime < puesto.darCarro().darTiempoEnParqueadero(horaActual)) {
                maxTime = puesto.darCarro().darTiempoEnParqueadero(horaActual - 1);
                placaMaxTime = puesto.darCarro().darPlaca();
            }
        }
        return "El auto que mas tiempo lleva es [" + placaMaxTime + "]" + "con [" + maxTime + "] horas";
    }
    ...
    public String metodo2() {
        String msgResult = "Hay puestos disponibles";
        if (Arrays.stream(puestos).allMatch(ocupado -> ocupado.darCarro() != null)) {
            msgResult = "El parqueadero está lleno";
        }
